...
#include "../sortingAlgorithm/bubbleSort.h"
...
void main(void)
{
  ...
  // This array is used to calc the averrage of an amount of attemps on the same algorithm operating on an array of specific size
  ...
  for(i=0; i<TESTS; i++)
  {
    for(j=0; j<REPETITIONS; j++) //we want the averrage of the total clock
    {
    ...
    //calc of time averrage and prepare the matrix
    for(j=1; j<REPETITIONS; j++)
      timeAverrage[i][0] += timeAverrage[i][j];
    timeAverrage[i][0] /= REPETITIONS;

    timeAverrage[i][1] = timeAverrage[i][0]; //time averrage on the second
    ...
    createArrayFile(timeAverrage, TESTS, 4, "array size ;clock averrage ;internal counter ;executions in 1 s\n", "Bubble sort");
  }
}
